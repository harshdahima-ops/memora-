...
function parseQuiz(text){
  try{
    ...
    for(const line of lines){
      ...
      else if(line.match(/^A[\)\.]/))a=line.replace(/^A[\)\.]s*/,'').replace(/^A\)\s*/,'').trim()
      else if(line.match(/^B[\)\.]/))b=line.replace(/^B[\)\.]s*/,'').replace(/^B\)\s*/,'').trim()
      else if(line.match(/^C[\)\.]/))c=line.replace(/^C[\)\.]s*/,'').replace(/^C\)\s*/,'').trim()
      else if(line.match(/^D[\)\.]/))d=line.replace(/^D[\)\.]s*/,'').replace(/^D\)\s*/,'').trim()
...
body{font-family:'Inter',sans-serif;background:${dark?'#0A0A0F':'#F0F0F8'};color:${dark?'#E8E8F0':'#0A0A1A'};-webkit-font-smoothing:antialiased;}
input,textarea,button,select{font-family:'Inter',sans-serif;}
input::placeholder,textarea::placeholder{color:${dark?'#3A3A50':'#9090B0'};}
...
::-webkit-scrollbar-thumb{background:${dark?'#2A2A3A':'#C0C0D8'};border-radius:10px;}
...
const C=(dark)=>({
  ...
  sidebar:dark?'#0F0F18':'#E8E8F5',
  card:dark?'#13131E':'#FFFFFF',
  card2:dark?'#1A1A28':'#EEEEF8',
  border:dark?'#20203A':'#D8D8EE',
  text:dark?'#E8E8F0':'#0A0A1A',
  muted:dark?'#6868A0':'#6060A0',
  accent:'#8B5CF6',
  accent2:'#7C3AED',
  green:'#10B981',
  red:'#EF4444',
...
function Av({name,size=36}){
  ...
  return <div style={{width:size,height:size,borderRadius:'50%',background:colors[idx],display:'flex',alignItems:'center',justifyContent:'center',fontSize:size*0.36,fontWeight:700,color:'#fff',flexShrink:0,letterSpacing:-0.5}}>{(name||'U').slice(0,2).toUpperCase()}</div>
}

function Chip({text,color}){
  const c=color||'#8B5CF6'
  return <span style={{fontSize:11,padding:'3px 10px',borderRadius:20,background:c+'20',color:c,fontWeight:600,flexShrink:0,letterSpacing:0.2}}>{text}</span>
}
...
function fmtInline(text,dark){
  if(!text)return''
  const c=C(dark)
  const cd={background:dark?'#1A1A28':'#EEEEF8',borderRadius:4,padding:'1px 6px',fontSize:'0.86em',fontFamily:'monospace',color:dark?'#A78BFA':'#7C3AED'}
  ...
  while(i<lines.length){
    ...
    if(l.startsWith('```')){
      if(!inCode){inCode=true;codeLines=[];lang=l.slice(3).trim()}
        ...
        els.push(<div key={i} style={{background:dark?'#080810':'#E8E8F5',borderRadius:10,overflow:'hidden',margin:'10px 0',border:`1px solid ${c.border}`}}>
          {lang&&<div style={{padding:'6px 14px',fontSize:11,color:c.muted,fontWeight:500,borderBottom:`1px solid ${c.border}`,background:dark?'#0F0F1A':'#EEEEF8'}}>{lang}</div>}
    ...
    if(l.startsWith('### '))els.push(<h3 key={i} style={{fontSize:14,fontWeight:600,margin:'8px 0 4px',color:c.text,letterSpacing:-0.2}}>{fmtInline(l.slice(4),dark)}</h3>)
    else if(l.startsWith('## '))els.push(<h2 key={i} style={{fontSize:16,fontWeight:700,margin:'12px 0 6px',color:c.text,letterSpacing:-0.3}}>{fmtInline(l.slice(3),dark)}</h2>)
    else if(l.startsWith('# '))els.push(<h1 key={i} style={{fontSize:18,fontWeight:700,margin:'14px 0 8px',color:c.text,letterSpacing:-0.5}}>{fmtInline(l.slice(2),dark)}</h1>)
    else if(l.startsWith('---'))els.push(<hr key={i} style={{border:'none',borderTop:`1px solid ${c.border}`,margin:'12px 0'}}/>)
    else if(l.startsWith('- ')||l.startsWith('* ')||l.startsWith('• '))els.push(<div key={i} style={{display:'flex',gap:8,marginBottom:4,paddingLeft:4}}><span style={{color:c.accent,marginTop:2,flexShrink:0}}>•</span><span style={{fontSize:14,color:c.muted,lineHeight:1.65}}>{fmtInline(l.replace(/^[-*•] /,''),dark)}</span></div>)
    else if(/^\d+\. /.test(l)){const[,n,tx]=l.match(/^(\d+)\. (.*)/);els.push(<div key={i} style={{display:'flex',gap:8,marginBottom:4,paddingLeft:4}}><span style={{color:c.accent,fontWeight:600,fontSize:13,flexShrink:0,minWidth:18}}>{n}.</span><span style={{fontSize:14,color:c.muted,lineHeight:1.65}}>{fmtInline(tx,dark)}</span></div>)}
    ...
    else els.push(<p key={i} style={{fontSize:14,color:c.muted,lineHeight:1.7,margin:'2px 0'}}>{fmtInline(l,dark)}</p>)
    i++
  }
...
function Landing({onAuth,dark}){
  ...
  const navFeatures=[
    {icon:'🎯',title:'Smart Study AI',desc:'Personalized for your exact board, exam and subject. From Class 5 to PhD.'},
    {icon:'💬',title:'General AI Chat',desc:'Not just study — chat, explore ideas, get help with anything in life.'},
    {icon:'🧩',title:'Interactive Quiz',desc:'One question at a time. See your score, track weak topics automatically.'},
  ...
  const stats=[['40+','Boards & Courses'],['₹99','Premium/Month'],['Free','To Start'],['24/7','Available']]

  if(view==='auth'){
    return(
      <div style={{minHeight:'100vh',background:'#0A0A0F',display:'flex',overflowY:'auto'}}>
        <style>{getCSS(true)}</style>
        {/* Left */}
        <div style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center',padding:40}}>
          <div style={{width:'100%',maxWidth:400}} className="fade-up">
            <button onClick={()=>setView('home')} style={{background:'none',border:'none',color:'#6868A0',fontSize:13,cursor:'pointer',marginBottom:28,display:'flex',alignItems:'center',gap:6,padding:0}}>← Back to home</button>
            <div style={{marginBottom:32}}>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:28,fontWeight:800,color:'#E8E8F0',marginBottom:6,letterSpacing:-0.5}}>
                {authMode==='login'?'Welcome back 👋':'Join Memora'}
              </div>
              <div style={{fontSize:14,color:'#6868A0'}}>{authMode==='login'?'Continue your learning journey':'Start studying smarter today'}</div>
            </div>

            <button onClick={googleLogin} disabled={loading} style={{width:'100%',display:'flex',alignItems:'center',justifyContent:'center',gap:12,padding:'13px',borderRadius:12,border:'1px solid #20203A',background:'#13131E',color:'#E8E8F0',fontSize:14,fontWeight:500,marginBottom:20,letterSpacing:0.1}}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:12,marginBottom:20}}>
              <div style={{flex:1,height:1,background:'#20203A'}}/><span style={{fontSize:12,color:'#3A3A58',whiteSpace:'nowrap'}}>or with email</span><div style={{flex:1,height:1,background:'#20203A'}}/>
            </div>

            <div style={{display:'flex',background:'#13131E',borderRadius:10,padding:3,marginBottom:20,border:'1px solid #20203A'}}>
              {['login','signup'].map(m=>(<button key={m} onClick={()=>{setAuthMode(m);setErr('')}} style={{flex:1,padding:'8px',borderRadius:8,border:'none',fontSize:13,fontWeight:500,background:authMode===m?'#8B5CF6':'transparent',color:authMode===m?'#fff':'#6868A0'}}>{m==='login'?'Login':'Sign Up'}</button>))}
            </div>

            {authMode==='signup'&&(
              <div style={{marginBottom:14}}>
                <label style={{fontSize:12,color:'#6868A0',display:'block',marginBottom:5,fontWeight:500}}>Your name</label>
                <input style={{width:'100%',padding:'11px 14px',borderRadius:10,border:'1px solid #20203A',background:'#13131E',color:'#E8E8F0',fontSize:14}} placeholder="e.g. Harsh" value={name} onChange={e=>setName(e.target.value)}/>
              </div>
            )}
            <div style={{marginBottom:14}}>
              <label style={{fontSize:12,color:'#6868A0',display:'block',marginBottom:5,fontWeight:500}}>Email address</label>
              <input style={{width:'100%',padding:'11px 14px',borderRadius:10,border:'1px solid #20203A',background:'#13131E',color:'#E8E8F0',fontSize:14}} type="email" placeholder="you@example.com" value={email} onChange={e=>setEmail(e.target.value)}/>
            </div>
            <div style={{marginBottom:22}}>
              <label style={{fontSize:12,color:'#6868A0',display:'block',marginBottom:5,fontWeight:500}}>Password</label>
              <input style={{width:'100%',padding:'11px 14px',borderRadius:10,border:'1px solid #20203A',background:'#13131E',color:'#E8E8F0',fontSize:14}} type="password" placeholder="min 6 characters" value={pass} onChange={e=>setPass(e.target.value)} onKeyDown={e=>e.key==='Enter'&&submit()}/>
            </div>

            {err&&<div style={{fontSize:13,color:'#EF4444',background:'#EF444415',border:'1px solid #EF444430',borderRadius:10,padding:'10px 14px',marginBottom:16,textAlign:'center'}}>{err}</div>}

            <button onClick={submit} disabled={loading} style={{width:'100%',padding:'13px',borderRadius:12,border:'none',background:loading?'#20203A':'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:14,fontWeight:700,letterSpacing:0.2}}>
              {loading?'Please wait...':(authMode==='login'?'Login to Memora':'Create Free Account')}
            </button>

            <div style={{textAlign:'center',marginTop:18,fontSize:13,color:'#6868A0'}}>
              {authMode==='login'?<>New here? <button onClick={()=>{setAuthMode('signup');setErr('')}} style={{background:'none',border:'none',color:'#8B5CF6',fontSize:13,fontWeight:600,cursor:'pointer'}}>Create free account</button></>:<>Already registered? <button onClick={()=>{setAuthMode('login');setErr('')}} style={{background:'none',border:'none',color:'#8B5CF6',fontSize:13,fontWeight:600,cursor:'pointer'}}>Login</button></>}
            </div>
          </div>
        </div>

        {/* Right - feature highlights */}
        <div style={{flex:1,background:'#0F0F18',borderLeft:'1px solid #20203A',display:'flex',alignItems:'center',justifyContent:'center',padding:40}}>
          <div style={{maxWidth:360}} className="fade-up-1">
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,color:'#E8E8F0',marginBottom:6}}>🧠 Memora</div>
            <div style={{fontSize:14,color:'#6868A0',lineHeight:1.7,marginBottom:28}}>Your AI-powered study companion. Personalized for every student in India.</div>
            {[['🎯','Covers Class 5–12, JEE, NEET, UPSC, SSC, Banking & 40+ courses'],['🧩','Interactive quiz with real-time scoring & weak topic tracking'],['📋','Upload your college syllabus — AI follows your exact curriculum'],['💬','Dual AI: Study mode + General conversation mode'],['⭐','Premium: Unlimited messages at just ₹99/month']].map(([ic,tx],i)=>(
              <div key={i} style={{display:'flex',alignItems:'flex-start',gap:12,padding:'10px 14px',background:'rgba(139,92,246,0.06)',border:'1px solid rgba(139,92,246,0.12)',borderRadius:10,marginBottom:8}}>
                <span style={{fontSize:18,flexShrink:0}}>{ic}</span>
                <span style={{fontSize:13,color:'#A0A0C8',lineHeight:1.55}}>{tx}</span>
              </div>
            ))}
          </div>
        </div>
  ...
  // Landing home
  return(
    <div style={{minHeight:'100vh',background:'#0A0A0F',color:'#E8E8F0',overflowY:'auto'}}>
      <style>{getCSS(true)}</style>
      {/* BG */}
      <div style={{position:'fixed',inset:0,overflow:'hidden',pointerEvents:'none',zIndex:0}}>
        <div style={{position:'absolute',top:'-15%',left:'50%',transform:'translateX(-50%)',width:700,height:700,background:'radial-gradient(ellipse,rgba(139,92,246,0.12) 0%,transparent 65%)'}}/>
        <div style={{position:'absolute',bottom:'-10%',right:'-5%',width:500,height:500,background:'radial-gradient(ellipse,rgba(16,185,129,0.06) 0%,transparent 65%)'}}/>
      </div>

      {/* Nav */}
      <nav style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'18px 40px',position:'relative',zIndex:10,borderBottom:'1px solid rgba(32,32,58,0.5)',backdropFilter:'blur(10px)'}}>
        ...
        <div style={{display:'flex',gap:10,alignItems:'center'}}>
          <button onClick={()=>{setView('auth');setAuthMode('login')}} style={{padding:'8px 20px',borderRadius:8,border:'1px solid #20203A',background:'transparent',color:'#A8A8C8',fontSize:13,fontWeight:500}}>Login</button>
          <button onClick={()=>{setView('auth');setAuthMode('signup')}} style={{padding:'8px 20px',borderRadius:8,border:'none',background:'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:13,fontWeight:600}}>Start Free</button>
        </div>
      </nav>

      {/* Hero */}
      <div style={{maxWidth:800,margin:'0 auto',textAlign:'center',padding:'80px 24px 60px',position:'relative',zIndex:10}}>
        <div className="fade-up" style={{display:'inline-flex',alignItems:'center',gap:8,background:'rgba(139,92,246,0.1)',border:'1px solid rgba(139,92,246,0.25)',borderRadius:20,padding:'6px 18px',fontSize:12,color:'#A78BFA',fontWeight:500,marginBottom:28,letterSpacing:0.3}}>
          ✦ AI-Powered Study Companion for Indian Students
        </div>
        <h1 className="fade-up-1" style={{fontFamily:"'Syne',sans-serif",fontSize:'clamp(40px,6vw,72px)',fontWeight:800,lineHeight:1.05,letterSpacing:-2,marginBottom:24}}>
          Study Smarter.<br/><span style={{background:'linear-gradient(135deg,#8B5CF6,#A78BFA)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent'}}>Score Higher.</span>
        </h1>
        <p className="fade-up-2" style={{fontSize:18,color:'#6868A0',lineHeight:1.7,marginBottom:40,maxWidth:560,margin:'0 auto 40px'}}>
          The most personalized AI study assistant for Indian students — from Class 5 to competitive exams to college. Built different.
        </p>
        <div className="fade-up-3" style={{display:'flex',gap:14,justifyContent:'center',flexWrap:'wrap',marginBottom:20}}>
          <button onClick={googleLogin} style={{display:'flex',alignItems:'center',gap:10,padding:'14px 26px',borderRadius:12,border:'1px solid #20203A',background:'#13131E',color:'#E8E8F0',fontSize:14,fontWeight:500}}>
            <svg width="18" height="18" viewBox="0 0 18 18"><path fill="#4285F4" d="M16.51 8H8.98v3h4.3c-.18 1-.74 1.48-1.6 2.04v2.01h2.6a7.8 7.8 0 002.38-5.88c0-.57-.05-.66-.15-1.18z"/><path fill="#34A853" d="M8.98 17c2.16 0 3.97-.72 5.3-1.94l-2.6-2.04a4.8 4.8 0 01-7.18-2.54H1.83v2.07A8 8 0 008.98 17z"/><path fill="#FBBC05" d="M4.5 10.48A4.8 4.8 0 014.5 9a5 5 0 01.32-1.76V5.17H2.18a8 8 0 000 7.66l2.32-2.35z"/><path fill="#EA4335" d="M8.98 4.72c1.3 0 2.11.56 2.6 1.03l1.93-1.93C12.07 2.79 10.6 2 8.98 2a8 8 0 00-6.8 3.83l2.32 2.07a4.77 4.77 0 014.48-3.18z"/></svg>
            Continue with Google
          </button>
          <button onClick={()=>{setView('auth');setAuthMode('signup')}} style={{padding:'14px 32px',borderRadius:12,border:'none',background:'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:14,fontWeight:700,letterSpacing:0.2}}>
            Start Free — No Card Needed
          </button>
        </div>
        <div className="fade-up-4" style={{fontSize:12,color:'#3A3A58'}}>20 free AI messages daily • No credit card required</div>
      </div>

      {/* Stats */}
      <div style={{display:'flex',justifyContent:'center',gap:60,flexWrap:'wrap',padding:'0 24px 60px',position:'relative',zIndex:10}}>
        {stats.map(([v,l],i)=>(<div key={i} style={{textAlign:'center'}}><div style={{fontFamily:"'Syne',sans-serif",fontSize:32,fontWeight:800,color:'#8B5CF6'}}>{v}</div><div style={{fontSize:12,color:'#3A3A58',marginTop:4,letterSpacing:0.3}}>{l}</div></div>))}
      </div>

      {/* Features */}
      <div style={{maxWidth:1000,margin:'0 auto',padding:'0 24px 80px',position:'relative',zIndex:10}}>
        <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:28,fontWeight:800,textAlign:'center',marginBottom:48,letterSpacing:-0.5}}>Everything you need to <span style={{color:'#8B5CF6'}}>ace your exams</span></h2>
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:20}}>
          {navFeatures.map((f,i)=>(
            <div key={i} style={{background:'#13131E',border:'1px solid #20203A',borderRadius:16,padding:'22px 22px',transition:'border-color 0.2s'}}>
              <div style={{fontSize:28,marginBottom:14}}>{f.icon}</div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:15,fontWeight:700,marginBottom:8,color:'#E8E8F0',letterSpacing:-0.2}}>{f.title}</div>
              <div style={{fontSize:13,color:'#6868A0',lineHeight:1.65}}>{f.desc}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* CTA */}
      <div style={{maxWidth:600,margin:'0 auto',textAlign:'center',padding:'0 24px 100px',position:'relative',zIndex:10}}>
        <div style={{background:'linear-gradient(135deg,rgba(139,92,246,0.1),rgba(124,58,237,0.05))',border:'1px solid rgba(139,92,246,0.2)',borderRadius:20,padding:'48px 40px'}}>
          <div style={{fontFamily:"'Syne',sans-serif",fontSize:26,fontWeight:800,marginBottom:12,letterSpacing:-0.5}}>Ready to study smarter?</div>
          <div style={{fontSize:14,color:'#6868A0',marginBottom:28,lineHeight:1.7}}>Join students who are already using Memora to prepare better and score higher.</div>
          <button onClick={()=>{setView('auth');setAuthMode('signup')}} style={{padding:'14px 36px',borderRadius:12,border:'none',background:'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:15,fontWeight:700,letterSpacing:0.2}}>
            Get Started Free
          </button>
        </div>
      </div>
    ...
    <div style={{minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:20,background:c.bg}}>
      ...
      <div style={{width:'100%',maxWidth:440}} className="fade-up">
        ...
        <div style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:18,padding:28}}>
          ...
          <button onClick={save} disabled={!board||loading} style={{width:'100%',padding:13,borderRadius:12,border:'none',background:!board||loading?c.border:'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:14,fontWeight:700,opacity:!board?0.5:1,letterSpacing:0.2}}>
            {loading?'Saving...':'Start Learning →'}
          </button>
        </div>
        <div style={{textAlign:'center',marginTop:16}}><button onClick={()=>onDone(null)} style={{background:'none',border:'none',color:c.muted,fontSize:13,cursor:'pointer'}}>Skip for now</button></div>
      </div>
    </div>
  )
}
...
function PremiumModal({onClose,dark,user}){
  ...
  const features=[{icon:'∞',label:'Unlimited AI messages every day'},{icon:'🧩',label:'Unlimited interactive quizzes'},{icon:'📁',label:'Upload images, PDFs & URLs'},{icon:'📋',label:'Syllabus PDF upload & analysis'},{icon:'⚡',label:'Priority AI response speed'},{icon:'📊',label:'Advanced progress analytics'}]

  return(
    <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.8)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20,overflowY:'auto'}}>
      <div style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:20,padding:32,width:'100%',maxWidth:440}} className="fade-up">
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:24}}>
          {features.map((f,i)=>(<div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'9px 14px',background:c.card2,borderRadius:10}}><span style={{fontSize:18,width:28,textAlign:'center'}}>{f.icon}</span><span style={{fontSize:13,color:c.text}}>{f.label}</span></div>))}
        </div>
        ...
        <div style={{display:'flex',gap:6,justifyContent:'center',flexWrap:'wrap',marginBottom:18}}>
          {['UPI','Credit Card','Debit Card','Net Banking','Wallets'].map(m=>(<span key={m} style={{fontSize:10,padding:'4px 10px',borderRadius:8,background:c.card2,border:`1px solid ${c.border}`,color:c.muted,fontWeight:500}}>{m}</span>))}
        </div>
        <button onClick={pay} disabled={loading} style={{width:'100%',padding:14,borderRadius:12,border:'none',background:loading?c.border:'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:15,fontWeight:700,marginBottom:10,letterSpacing:0.2}}>
...
function QuizCard({quiz,onAnswer,onNext,onEnd,score,total,dark}){
  ...
  return(
    <div style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:16,padding:'20px 18px',maxWidth:'88%',marginTop:10}} className="msg-in">
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16}}>
        <div style={{display:'flex',alignItems:'center',gap:8}}>
          ...
          <div style={{height:3,width:80,background:c.card2,borderRadius:2,overflow:'hidden'}}>
            <div style={{height:'100%',width:Math.min((score/(total||1))*100,100)+'%',background:'#10B981',borderRadius:2,transition:'width 0.4s'}}/>
          </div>
        </div>
      ...
      <div style={{display:'flex',flexDirection:'column',gap:9,marginBottom:16}}>
        {Object.entries(quiz.options).map(([letter,text])=>{
          ...
          if(revealed){
            if(letter===quiz.correct){bg='rgba(16,185,129,0.12)';border='#10B981';col='#10B981'}
            else if(sel===letter){bg='rgba(239,68,68,0.12)';border='#EF4444';col='#EF4444'}
          }else if(sel===letter){bg='rgba(139,92,246,0.12)';border='#8B5CF6';col='#8B5CF6'}
          return(
            <button key={letter} onClick={()=>!revealed&&setSel(letter)} style={{padding:'11px 16px',borderRadius:10,border:`1px solid ${border}`,background:bg,color:col,fontSize:14,textAlign:'left',cursor:revealed?'default':'pointer',transition:'all 0.15s',fontWeight:sel===letter?600:400,display:'flex',alignItems:'center',gap:10}}>
              <span style={{width:22,height:22,borderRadius:'50%',border:`1.5px solid ${border}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,flexShrink:0,color:col}}>{letter}</span>
              {text}
              {revealed&&letter===quiz.correct&&<span style={{marginLeft:'auto',fontSize:16}}>✅</span>}
              {revealed&&sel===letter&&letter!==quiz.correct&&<span style={{marginLeft:'auto',fontSize:16}}>❌</span>}
            </button>
          )
        })}
      </div>
      {!revealed?(
        <button onClick={submit} disabled={!sel} style={{width:'100%',padding:'11px',borderRadius:10,border:'none',background:sel?'linear-gradient(135deg,#8B5CF6,#7C3AED)':c.border,color:'#fff',fontSize:14,fontWeight:700,opacity:sel?1:0.5}}>Submit Answer</button>
      ):(
        <div>
          <div style={{padding:'12px 16px',borderRadius:10,background:sel===quiz.correct?'rgba(16,185,129,0.1)':'rgba(239,68,68,0.1)',border:`1px solid ${sel===quiz.correct?'rgba(16,185,129,0.3)':'rgba(239,68,68,0.3)'}`,marginBottom:12,textAlign:'center'}}>
            <div style={{fontSize:15,fontWeight:700,color:sel===quiz.correct?'#10B981':'#EF4444'}}>{sel===quiz.correct?'🎉 Correct! Well done.':'❌ Wrong answer!'}</div>
            {sel!==quiz.correct&&<div style={{fontSize:13,color:c.muted,marginTop:4}}>Correct: <strong style={{color:'#10B981'}}>{quiz.correct}) {quiz.options[quiz.correct]}</strong></div>}
          </div>
...
function ChatTab({user,notes,profile,onSaveNote,weakTopics,setWeakTopics,isPremium,dark,onUpgrade,aiMode}){
  ...
  function buildSystem(){
    if(aiMode==='general'){
      return 'You are Memora, a helpful and friendly AI assistant. You can help with anything — answer questions, have conversations, help with creative tasks, give advice, or just chat. Be warm, helpful and engaging. Format responses clearly with markdown where useful.'
    }
    ...
    sys+=' Rules: ## for unit headings, **bold** key terms, numbered lists for steps, - for bullet points. Be exam-focused, no padding or filler content.'
    ...
    if(mode==='flashcard')sys+=' Task: Generate 5 flashcard-style Q&A pairs on the topic. Format as:\nQ: [question]\nA: [concise answer]\n\nRepeat for each card.'
    ...
    if(mode==='predict')sys+=' Task: Based on the topic and standard exam patterns, predict the most likely exam questions. List top 5-8 probable questions with brief hints.'
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'20px 16px'}}>
        {loadingMsgs&&<div style={{textAlign:'center',padding:40}}><div style={{fontSize:28,animation:'pulse 1.5s ease infinite'}}>🧠</div></div>}
        {!loadingMsgs&&messages.length===0&&(
          <div style={{textAlign:'center',padding:'48px 16px',color:c.muted}}>
            ...
            <div style={{fontSize:14,lineHeight:1.7,marginBottom:28,maxWidth:440,margin:'0 auto 28px'}}>
              {aiMode==='general'?'Ask me anything — I can help with any topic, answer questions, or just have a conversation.':'Ask anything from your syllabus. I\'ll explain concepts, make notes, quiz you, or predict exam questions.'}
            </div>
            <div style={{display:'flex',gap:8,justifyContent:'center',flexWrap:'wrap'}}>
              {(aiMode==='general'?["What's quantum computing?","Write a poem about India","Help me with a cover letter"]:["Explain Newton's laws simply","Summarize the water cycle","Quiz me on Photosynthesis","Predict exam questions for Trigonometry"]).map(s=>(<button key={s} onClick={()=>setInput(s)} style={{padding:'7px 14px',borderRadius:20,border:`1px solid ${c.border}`,background:'transparent',color:c.muted,fontSize:12,fontWeight:400}}>{s}</button>))}
            </div>
          </div>
        )}
        ...
        {messages.map(m=>(
          <div key={m.id} style={{display:'flex',gap:10,marginBottom:18,flexDirection:m.role==='user'?'row-reverse':'row',alignItems:'flex-start'}} className="msg-in">
            {m.role==='ai'&&<div style={{flexShrink:0,marginTop:2}}><div style={{width:28,height:28,borderRadius:'50%',background:'linear-gradient(135deg,#8B5CF6,#7C3AED)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:14}}>🧠</div></div>}
            <div style={{maxWidth:'80%',display:'flex',flexDirection:'column',alignItems:m.role==='user'?'flex-end':'flex-start'}}>
              ...
              {m.role==='ai'&&<button onClick={()=>onSaveNote(m.content)} style={{fontSize:11,color:'#8B5CF6',background:'none',border:'none',padding:'5px 0 0',cursor:'pointer',textAlign:'left',opacity:0.8}}>＋ Save as note</button>}
            </div>
          </div>
        ))}
      ...
      {/* Attachment preview */}
      {attachment&&(<div style={{padding:'6px 16px',borderTop:`1px solid ${c.border}`,background:c.card2,display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:12,color:'#8B5CF6'}}>📎 {attachment.type==='image'?'Image':'URL'}: {attachment.preview.slice(0,50)}</span><button onClick={()=>setAttachment(null)} style={{background:'none',border:'none',color:c.red,fontSize:14,padding:0}}>✕</button></div>)}
      ...
      <div style={{padding:'12px 16px 16px',borderTop:`1px solid ${c.border}`,background:c.bg,flexShrink:0}}>
        {limitHit&&(<div style={{fontSize:13,color:c.red,background:'rgba(239,68,68,0.08)',border:'1px solid rgba(239,68,68,0.2)',borderRadius:8,padding:'8px 14px',marginBottom:10,textAlign:'center',cursor:'pointer'}} onClick={onUpgrade}>Daily limit reached. <span style={{textDecoration:'underline',fontWeight:700}}>Upgrade to Premium ⭐</span></div>)}
        {aiMode==='study'&&(
        ...
        <div style={{display:'flex',gap:8,alignItems:'flex-end'}}>
          ...
          <textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} placeholder={mode==='quiz'?'Enter topic to start quiz (e.g. Photosynthesis)...':aiMode==='general'?'Ask me anything...':'Ask anything from your syllabus...'} disabled={limitHit} rows={1} style={{flex:1,padding:'11px 14px',borderRadius:10,border:`1px solid ${c.border}`,background:c.card,color:c.text,fontSize:14,resize:'none',maxHeight:120,lineHeight:1.5,overflowY:'auto'}} onInput={e=>{e.target.style.height='auto';e.target.style.height=Math.min(e.target.scrollHeight,120)+'px'}}/>
          <button onClick={send} disabled={typing||!input.trim()||limitHit} style={{padding:'11px 20px',borderRadius:10,border:'none',background:typing||!input.trim()||limitHit?c.border:'linear-gradient(135deg,#8B5CF6,#7C3AED)',color:'#fff',fontSize:13,fontWeight:700,flexShrink:0,opacity:typing||!input.trim()||limitHit?0.5:1,letterSpacing:0.2}}>Send</button>
        </div>
...
function NotesTab({user,notes,setNotes,prefill,clearPrefill,dark}){
  ...
  const filtered=notes.filter(n=>{
    const matchesFilter=!filter.trim()||n.title.toLowerCase().includes(filter.toLowerCase())||n.body.toLowerCase().includes(filter.toLowerCase())
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'14px 16px'}}>
        ...
        <div style={{display:'grid',gap:12}}>
          {filtered.map(n=>(
            <div key={n.id} style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:14,overflow:'hidden',transition:'border-color 0.15s'}}>
              <div style={{padding:'14px 16px',cursor:'pointer'}} onClick={()=>setExpanded(expanded===n.id?null:n.id)}>
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10}}>
                  <div style={{flex:1}}>
                    <div style={{fontWeight:600,fontSize:14,color:c.text,marginBottom:4,lineHeight:1.4}}>{n.title}</div>
                    {expanded!==n.id&&<div style={{fontSize:12,color:c.muted,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',lineHeight:1.5}}>{n.body.replace(/[#*`_]/g,'').slice(0,100)}</div>}
                  </div>
      ...
      {showModal&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:999,padding:20}}>
          <div style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:18,padding:28,width:'100%',maxWidth:460}} className="fade-up">
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:800,marginBottom:20,color:c.text,letterSpacing:-0.3}}>Save Note 📝</div>
            ...
            <div style={{marginBottom:22}}>
              <label style={{fontSize:11,color:c.muted,display:'block',marginBottom:5,fontWeight:500,textTransform:'uppercase',letterSpacing:0.4}}>Content <span style={{fontSize:10,textTransform:'none',letterSpacing:0}}>— supports **bold**, *italic*, ## headings, - lists</span></label>
...
function ProgressTab({user,profile,weakTopics,setWeakTopics,notes,dark,onUpgrade,isPremium}){
  ...
  return(
    <div style={{flex:1,overflowY:'auto',padding:'16px'}}>
      ...
      {/* Premium upgrade */}
      {!isPremium&&(
        <div style={{background:'linear-gradient(135deg,rgba(139,92,246,0.1),rgba(124,58,237,0.05))',border:'1px solid rgba(139,92,246,0.2)',borderRadius:14,padding:'18px 18px',marginBottom:14,cursor:'pointer'}} onClick={onUpgrade}>
    ...
    <div style={{display:'flex',flexDirection:'column',height:'100%',overflow:'hidden'}}>
      <div style={{padding:'16px',borderBottom:`1px solid ${c.border}`,flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',gap:12,background:c.card,border:`1px solid ${c.border}`,borderRadius:12,padding:'11px 16px'}}>
          ...
          {q&&<button onClick={()=>setQ('')} style={{background:'none',border:'none',color:c.muted,fontSize:18,padding:0,lineHeight:1}}>✕</button>}
        </div>
        {q&&<div style={{fontSize:12,color:c.muted,marginTop:8}}>{results.length} result{results.length!==1?'s':''} found</div>}
      </div>
      <div style={{flex:1,overflowY:'auto',padding:'14px 16px'}}>
        {!q&&<div style={{textAlign:'center',padding:'60px 16px',color:c.muted}}><div style={{fontSize:40,marginBottom:14}}>🔍</div><div style={{fontSize:16,fontWeight:600,color:c.text,marginBottom:6}}>Search your notes</div><div style={{fontSize:13}}>Find anything you've saved across all your notes.</div></div>}
          ...
          {results.map((r,i)=>(
            <div key={i} style={{background:c.card,border:`1px solid ${c.border}`,borderRadius:12,padding:'14px 16px'}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:6}}>
                <Chip text={r.tag} color='#8B5CF6'/>
              </div>
              <div style={{fontWeight:600,fontSize:14,marginBottom:6,color:c.text,lineHeight:1.4}}>{hl(r.title,q)}</div>
...
function Sidebar({tab,setTab,user,notes,dark,setDark,onLogout,onUpgrade,isPremium,aiMode,setAiMode}){
  ...
  return(
    <div style={{width:240,height:'100%',background:c.sidebar,borderRight:`1px solid ${c.border}`,display:'flex',flexDirection:'column',flexShrink:0}}>
      {/* Logo */}
      <div style={{padding:'20px 18px 16px',borderBottom:`1px solid ${c.border}`}}>
        ...
        <div style={{fontSize:11,color:c.muted,marginTop:3,letterSpacing:0.2}}>AI Study Companion</div>
      </div>
      ...
      <div style={{flex:1,padding:'4px 8px',overflowY:'auto'}}>
        {navItems.map(item=>(
          <button key={item.id} onClick={()=>setTab(item.id)} style={{width:'100%',padding:'10px 12px',borderRadius:10,border:'none',background:tab===item.id?'rgba(139,92,246,0.12)':'transparent',color:tab===item.id?'#8B5CF6':c.muted,fontSize:13,fontWeight:tab===item.id?600:400,display:'flex',alignItems:'center',gap:10,marginBottom:2,textAlign:'left',letterSpacing:tab===item.id?-0.1:0}}>
        ...
        {!isPremium?(
          <button onClick={onUpgrade} style={{width:'100%',padding:'10px 12px',borderRadius:10,border:'1px solid rgba(139,92,246,0.25)',background:'linear-gradient(135deg,rgba(139,92,246,0.08),rgba(124,58,237,0.04))',color:'#8B5CF6',fontSize:13,fontWeight:600,display:'flex',alignItems:'center',gap:10,textAlign:'left',marginTop:4}}>
            <span style={{fontSize:16}}>⭐</span>Upgrade to Pro
            <span style={{marginLeft:'auto',fontSize:10,background:'rgba(139,92,246,0.2)',borderRadius:8,padding:'1px 6px'}}>₹99</span>
          </button>
        ):(
          <div style={{margin:'4px',padding:'10px 12px',borderRadius:10,background:'rgba(245,158,11,0.08)',border:'1px solid rgba(245,158,11,0.2)',fontSize:12,color:'#F59E0B',fontWeight:600,display:'flex',alignItems:'center',gap:8}}>
      ...
      <div style={{padding:'10px 8px 14px',borderTop:`1px solid ${c.border}`}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10,padding:'6px 4px'}}>
          ...
          <div style={{flex:1,overflow:'hidden'}}>
            <div style={{fontSize:13,fontWeight:600,color:c.text,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',letterSpacing:-0.2}}>{userName}</div>
...
function BottomNav({tab,setTab,notes,dark}){
  ...
  return(
    <div style={{display:'flex',borderTop:`1px solid ${c.border}`,background:c.sidebar,flexShrink:0}}>
      {items.map(item=>(<button key={item.id} onClick={()=>setTab(item.id)} style={{flex:1,padding:'10px 4px 8px',border:'none',borderTop:`2px solid ${tab===item.id?'#8B5CF6':'transparent'}`,background:'transparent',color:tab===item.id?'#8B5CF6':c.muted,fontSize:10,fontWeight:tab===item.id?700:400,display:'flex',flexDirection:'column',alignItems:'center',gap:2,position:'relative',letterSpacing:0.1}}><span style={{fontSize:20}}>{item.icon}</span>{item.label}{item.badge>0&&<span style={{position:'absolute',top:6,right:'calc(50% - 18px)',fontSize:9,background:'#8B5CF6',color:'#fff',borderRadius:10,padding:'1px 5px',fontWeight:700}}>{item.badge}</span>}</button>))}
    </div>
  )
}
...
export default function App(){
  ...
  if(loading)return <div style={{minHeight:'100vh',background:'#0A0A0F',display:'flex',alignItems:'center',justifyContent:'center'}}><style>{getCSS(dark)}</style><div style={{fontSize:44,animation:'pulse 1.5s ease infinite'}}>🧠</div></div>
